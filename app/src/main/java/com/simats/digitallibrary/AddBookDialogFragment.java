...
/**
 * Dialog Fragment for Adding a New Book
 */
public class AddBookDialogFragment extends DialogFragment {
    ...
    private void saveBook() {
        ...
        // First upload image if selected, then save book
        uploadCoverImage(coverUrl -> {
            // Build JSON request
            ...
            btnSaveBook.setText("Saving...");

            JsonObjectRequest request = new JsonObjectRequest(
                    Request.Method.POST,
                    ApiConfig.URL_ADD_BOOK,
                    bookData,
                    response -> {
                        try {
                            if (response.getBoolean("success")) {
                                ...
                                String message = response.optString("message", "Failed to add book");
    ...
    private void onImageSelected(Uri uri) {
        ...
        tvImageFileName.setText(fileName);
