...
public class SearchBooksActivity extends AppCompatActivity {
    ...
    private boolean skipTextWatcher = false; // Flag to skip TextWatcher when category is set
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Check if category filter passed - use EXACT category match
        String category = getIntent().getStringExtra("category");
        if (category != null && !category.isEmpty()) {
            // Set flag to prevent TextWatcher from triggering NLP search
            skipTextWatcher = true;
            etSearch.setText(category);
            // Use exact category filter instead of NLP search
    ...
    private void setupSearch() {
        etSearch.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                // Skip if category was set programmatically
                if (skipTextWatcher) {
    ...
    /**
     * Load books by EXACT category match
     * Uses get_books.php?category=X for exact filtering
     */
    private void loadBooksByCategory(String category) {
        ...
        String url = ApiConfig.URL_GET_BOOKS + "?category=" + encodedCategory;
        ...
        JsonObjectRequest request = new JsonObjectRequest(
                ...
                response -> {
                    ...
                    try {
                        if (response.getBoolean("success")) {
                            ...
                            if (books.isEmpty()) {
                                tvResultsCount.setText("📚 " + category);
                                showNoResults(category);
                            } else {
                                tvResultsCount.setText("📚 " + category + " (" + books.size() + " books)");
                                adapter.setBooks(books);
                                recyclerBooks.setVisibility(View.VISIBLE);
                            }
                        } else {
                ...
                error -> {
                    progressBar.setVisibility(View.GONE);
                    Log.e(TAG, "Error loading category: " + error.getMessage());
                    showNoResults(category);
                });
